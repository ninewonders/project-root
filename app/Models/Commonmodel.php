...
class Commonmodel extends Model {
    ...
    public function initialize($tablename, $fields) {
    }
    ...
    function get_site_blog_posts($total, $limit, $offset){
        ...
        function get_site_blog_tools(){
        //$this->db->select('b.*');
        $this->db->select('c.title-en as category, b.title as title');
        $this->db->from('businesses b');
        $this->db->where('b.lang', 'en');
        //$this->db->where('u.role', 'admin');
        //$this->db->where('b.status', 1);
        /*if(isset($_GET['search']) && $_GET['search'] != ''){
            $this->db->like('b.title', $_GET['search']);
       ...
       // $this->db->join('users u', 'u.id = b.user_id', 'LEFT');
        $this->db->join('bus-category c', 'c.id = b.id-cat', 'LEFT');
     ...
     function get_one_site_blog_tools($slug){
        //echo $slug;
       $lang =get_lang();
        $this->db->select('id,title,content,image,meta-title as  meta_title,meta-desc as  meta_desc,meta-key as  meta_key, lang, slug');
        $this->db->from('businesses');
        $this->db->where('slug', html_entity_decode($slug, ENT_COMPAT, 'UTF-8'));
            $query = $this->db->get();
            $query = $query->row();
           // print_r($this->db->last_query());  die;
           
            if( substr($lang,0,2) == $query->lang or $lang==NULL){
                return $query;
            }else{
               
                $condi='id_business_'.substr($lang,0,2);
                $condi2 = 'id_business_'.$query->lang;
                 $this->db->select( $condi .' as uid');
        $this->db->from('lang-corespondance');
         $this->db->where($condi2, $query->id);   
         $query2 = $this->db->get();
        $query2 = $query2->row();
        //print_r($this->db->last_query());die;
            
        
                //$condi='c.id_business_'.substr($lang,0,2);
                $this->db->select('b. id,b.title,b.content,b.image,b.meta-title as  meta_title,b.meta-desc as  meta_desc,b.meta-key as  meta_key, b.lang ,b.slug');
        $this->db->from('businesses b');
        
        $this->db->where('id', $query2->uid );
        
       // $this->db->join('lang-corespondance c', $condi .'=b.id', 'LEFT');
            $query3 = $this->db->get();
            $query3 = $query3->row();
            //print_r($this->db->last_query());die;
             //header("location:https://datetric.com/scheduling-tools/".$query->slug);
            // redirect('/scheduling-tools/'.$query->slug);
          // $query3->append($query->lang) ;
           //$arrayobj = new ArrayObject($query3);
           //$arrayobj->append($query->lang);
           $table=array();
           $table['lang']=$query->lang;
           $table['slug']=$query3->slug;
          // var_dump($table); die;
            return $table ;
            }
  ...
  //get posts categories tools
    function get_category_tools()
    {
        //echo "dfdfdf";die();
        $this->db->select();
        $this->db->from('bus-category');
